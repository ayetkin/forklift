...
type (
	...
	Client interface {
		...
		Exit(reason error) int
		RunConsumers() error
		AddConsumer(queueName string) *Consumer
		AddPublisher(exchangeName string, exchangeType ExchangeType, payloads ...interface{})
...
func NewRabbitMqClient(nodes []string, userName string, password string, virtualHost string, withFunc ...withFunc) Client {
	...
	for _, handler := range withFunc {
		if err := handler(clientObj); err != nil {
			panic(err)
		}
	}
	return clientObj
